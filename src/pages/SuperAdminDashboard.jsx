    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">System-wide overview — you can see everything</p>
        </div>
      ...
      {!loading && stats?.totalTasks > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
          <h2 className="text-base font-semibold text-gray-900 mb-4">System Task Distribution</h2>
          <div className="flex h-4 rounded-full overflow-hidden gap-0.5 mb-3">
            {[
              ...
              { count: stats.notStartedTasks,  color: 'bg-gray-200',   label: 'Not Started' },
          ...
          <div className="flex flex-wrap gap-4 text-xs text-gray-500">
            {[
              ...
              { label: 'Not Started',  count: stats.notStartedTasks,  dot: 'bg-gray-300' },
            ].map(({ label, count, dot }) => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-semibold text-gray-900">Recent Task Activity</h2>
          <Link to="/admin/employees" className="text-sm text-blue-600 hover:text-blue-700 font-medium">
            View all employees →
          </Link>
        </div>
        {loading ? (
          ...
          <div className="text-center py-10">
            <p className="text-gray-400 text-sm">No tasks in the system yet</p>
          </div>
        ) : (
          <div className="space-y-2">
            {data.recentTasks.map((task) => (
              <div key={task._id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                ...
                <p className={`text-xs hidden sm:block ${task.status === 'overdue' ? 'text-red-500 font-medium' : 'text-gray-400'}`}>
