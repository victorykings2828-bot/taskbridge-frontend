...
const StarDisplay = ({ rating }) => (
  <span className="text-amber-400 font-semibold text-sm">
    ...
    <span className="text-gray-500 font-normal ml-1 text-xs">{rating}</span>
  </span>
);
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <button onClick={() => navigate(-1)} className="text-sm text-gray-400 hover:text-gray-600 mb-1 flex items-center gap-1">← Back</button>
          <h1 className="text-2xl font-bold text-gray-900">All Employees</h1>
          <p className="text-gray-500 mt-1">{employees.length} employee{employees.length !== 1 ? 's' : ''} across all teams</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="bg-white rounded-2xl p-6"><TableSkeleton rows={8} /></div>
      ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-left text-xs font-semibold text-gray-500 px-5 py-3 hidden md:table-cell">Manager</th>
                    <th className="text-left text-xs font-semibold text-gray-500 px-5 py-3">Tasks</th>
                    <th className="text-left text-xs font-semibold text-gray-500 px-5 py-3 hidden sm:table-cell">Done Rate</th>
                    <th className="text-left text-xs font-semibold text-gray-500 px-5 py-3 hidden lg:table-cell">Avg Rating</th>
                    ...
                    return (
                      <tr key={emp._id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-5 py-3 hidden md:table-cell">
                          <p className="text-sm text-gray-600">{emp.managerId?.name || '—'}</p>
                        </td>
                        <td className="px-5 py-3">
                        ...
                        <td className="px-5 py-3 hidden sm:table-cell">
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-gray-500">{emp.stats.completionRate}%</span>
                            </div>
                            <p className="text-xs text-gray-400 mt-0.5">{emp.stats.completedTasks}/{emp.stats.totalTasks} tasks</p>
                          </div>
                        </td>
