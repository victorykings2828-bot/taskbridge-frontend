  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <button onClick={() => window.history.back()} className="text-sm text-gray-400 hover:text-gray-600 mb-1 flex items-center gap-1">← Back</button>
          <h1 className="text-2xl font-bold text-gray-900">All Managers</h1>
          <p className="text-gray-500 mt-1">{managers.length} manager{managers.length !== 1 ? 's' : ''} in the system</p>
        </div>
      ...
      {loading ? <div className="bg-white rounded-2xl p-6"><TableSkeleton rows={6} /></div> : (
              ...
              <Link key={mgr._id} to={`/admin/managers/${mgr._id}`}
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-primary">{mgr.name}</p>
                ...
                <div className="grid grid-cols-4 gap-2 text-center">
                  {[
                    ...
                    { label: 'Tasks',     value: mgr.stats.totalTasks,     color: 'text-gray-700' },
                ...
                <div className="mt-3">
                  <div className="flex justify-between text-xs text-gray-400 mb-1">
                    ...
                    <span className="font-medium text-gray-600">{mgr.stats.completionRate}%</span>
                  </div>
                ...
                <p className="text-xs text-gray-400 mt-3">Joined {formatDate(mgr.createdAt)}</p>
              </Link>
            );
          })}
          {filtered.length === 0 && (
